...
def main():
    ...
    with open("../../res_" + fname + ".json", "r") as json_file:
        data = json.load(json_file)
        prompt_to_res = {}
        ptype = args.prompt_type
        for item in data:
            prompt_to_res[item[ptype]] = item[ptype + "_response"]
        
        for line in lines:
            res = prompt_to_res[line]
            if res.strip() == "":
                res = "Sorry, I cannot fullfill your request."
            outputs.append(res)

    results = pd.DataFrame()
    results["prompt"] = lines
    results["output"] = outputs
    results.to_csv(f"outputs/{fname}/output_{args.prompt_type}_{str(args.subjects)}.csv")
