...
def run(
    ...
    if "default" in config_set:
        ...
        res = pd.read_csv(f"./outputs/{file_name}/output_{args.prompt_type}_{str(args.subjects)}.csv")
    ...
    json.dump(
        summary,
        open(f"merged_eval_results/{args.config}/{model_name}_{prompt_type}_{str(subjects)}_summary.json", "w"),
        indent=4,
    )
