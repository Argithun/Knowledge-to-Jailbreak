...
def pick_best_and_eval(res, scorer, evaluator, n_sample, prompts, matching_only=False):
    ...
    res_best["output"] = res["output"] #best_texts
...
def run(
    ...
    n_eval=100,
    matching_only=False,
    evaluator_path="/data3/MODELS/llama2-hf/llama-2-13b-chat",
    scorer_path="./scorer/scorer/pytorch_model.bin",
    prompt_type='original_prompt',
    subjects=[]
):
    summary = {}
    ...
    scorer_model = AutoModelForSequenceClassification.from_pretrained("/home/tsq/pzr_use/code_for_eassy/main_experiment/cal_ASR/Jailbreak_LLM-main/scorer")
    evaluator_model = AutoModelForSequenceClassification.from_pretrained("/home/tsq/pzr_use/code_for_eassy/main_experiment/cal_ASR/Jailbreak_LLM-main/evaluator")
    ...
    if "default" in config_set:
        ...
        res = pd.read_csv(f"./outputs/{file_name}/output_{args.prompt_type}_{str(args.subjects)}_chem.csv")
    ...
    print(f"{model_name}_{prompt_type}_{str(subjects)}: Done!")
    json.dump(
        summary,
        open(f"merged_eval_results/{args.config}/{model_name}_{prompt_type}_{str(subjects)}_summary_chem.json", "w"),
        indent=4,
    )
